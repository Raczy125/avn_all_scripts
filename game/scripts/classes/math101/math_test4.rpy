...
$ mathTestScore = 0
$ math_questions = 10
$ math_answers = ["b","d","a","d","b","a","c","b","d","a"]
$ math_test_name = "images/classes/math101/test04/math04_test%d.jpg"
$ count = 1
...
if mathResult + bonusPercentageMath >= 90 - mathBoost:
    $ passedMath += 1
    if not persistent.ep4_card17:
        $ chat_new_rewards = True
        $ persistent.rew_sage_unlocked += 1
        $ persistent.ep4_card17 = True
        $ calcRenders()
...
if steamAchievements and mathResult + bonusPercentageMath >= 70 - mathBoost and not config.console and not config.developer:
    ...
    $ achievement.Sync()
if steamAchievements and not config.console and not config.developer and maxedMath == 4:
    $ achievement.grant("WHIZKID")
    init:
        $ achievement.register("WHIZKID")
$ achievement.Sync()
